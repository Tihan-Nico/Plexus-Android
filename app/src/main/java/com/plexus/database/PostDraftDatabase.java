...
public class PostDraftDatabase extends Database {

    public static final String TABLE_NAME  = "drafts";
    public static final String ID          = "_id";
    public static final String POST_ID   = "thread_id";
    public static final String DRAFT_TYPE  = "type";
    public static final String DRAFT_VALUE = "value";

    public static final String CREATE_TABLE = "CREATE TABLE " + TABLE_NAME + " (" + ID + " INTEGER PRIMARY KEY, " +
            POST_ID + " TEXT, " + DRAFT_TYPE + " TEXT, " + DRAFT_VALUE + " TEXT + );";

    public static final String[] CREATE_INDEXS = {
            "CREATE INDEX IF NOT EXISTS draft_thread_index ON " + TABLE_NAME + " (" + POST_ID + ");",
    };

    public PostDraftDatabase(Context context, SQLCipherOpenHelper databaseHelper) {
        super(context, databaseHelper);
    }

    public void insertDrafts(long threadId, List<DraftDatabase.Draft> drafts) {
        SQLiteDatabase db    = databaseHelper.getWritableDatabase();

        for (DraftDatabase.Draft draft : drafts) {
            ContentValues values = new ContentValues(3);
            values.put(POST_ID, threadId);
    ...
    public void clearDrafts(long threadId) {
        SQLiteDatabase db = databaseHelper.getWritableDatabase();
        db.delete(TABLE_NAME, POST_ID + " = ?", new String[] {threadId+""});
    }
    ...
    public List<DraftDatabase.Draft> getDrafts(long threadId) {
        SQLiteDatabase db   = databaseHelper.getReadableDatabase();
        List<DraftDatabase.Draft> results = new LinkedList<>();
        Cursor cursor       = null;

        try {
            cursor = db.query(TABLE_NAME, null, POST_ID + " = ?", new String[] {threadId+""}, null, null, null);
            ...
            while (cursor != null && cursor.moveToNext()) {
                ...
                results.add(new DraftDatabase.Draft(type, value));
            }
    ...
    public static class Drafts extends LinkedList<DraftDatabase.Draft> {
        private DraftDatabase.Draft getDraftOfType(String type) {
            for (DraftDatabase.Draft draft : this) {
        ...
        public String getSnippet(Context context) {
            DraftDatabase.Draft textDraft = getDraftOfType(DraftDatabase.Draft.TEXT);
            if (textDraft != null) {
        ...
        public @Nullable
        Uri getUriSnippet() {
            DraftDatabase.Draft imageDraft = getDraftOfType(DraftDatabase.Draft.IMAGE);
