...
import com.plexus.utils.JsonUtil;
import com.plexus.utils.JsonUtils;
import com.plexus.utils.MediaUtil;
import com.plexus.utils.SetUtil;
import com.plexus.utils.StorageUtil;
...
public class AttachmentDatabase extends Database {
    ...
    public void trimAllAbandonedAttachments() {
        SQLiteDatabase db = databaseHelper.getWritableDatabase();
        String selectDataInUse = "SELECT DISTINCT " + DATA + " FROM " + TABLE_NAME + " WHERE " + QUOTE + " = 0 AND ("  + MMS_ID + " = " + PREUPLOAD_MESSAGE_ID + ")";
        String where = MMS_ID + DATA + " NOT IN (" + selectDataInUse + ")";
    ...
    public void copyAttachmentData(@NonNull AttachmentId sourceId, @NonNull AttachmentId destinationId)
        ...
        if (sourceAttachment == null) {
            throw new DatabaseException("Cannot find attachment for source!");
        }
        ...
        if (sourceDataInfo == null) {
            throw new DatabaseException("No attachment data found for source!");
        }
    ...
    public @NonNull
    DatabaseAttachment insertAttachmentForPreUpload(@NonNull Attachment attachment) throws DatabaseException {
        Map<Attachment, AttachmentId> result = insertAttachmentsForMessage(PREUPLOAD_MESSAGE_ID,
                Collections.singletonList(attachment),
                Collections.emptyList());

        if (result.values().isEmpty()) {
            throw new DatabaseException("Bad attachment result!");
        }

        DatabaseAttachment databaseAttachment = getAttachment(result.values().iterator().next());

        if (databaseAttachment == null) {
            throw new DatabaseException("Failed to retrieve attachment we just inserted!");
        }
    ...
    public void updateAttachmentData(@NonNull DatabaseAttachment databaseAttachment,
        ...
        if (oldDataInfo == null) {
            throw new DatabaseException("No attachment data found!");
        }
