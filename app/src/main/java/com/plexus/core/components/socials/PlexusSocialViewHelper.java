package com.plexus.core.components.socials;
...
public final class PlexusSocialViewHelper implements PlexusSocialView {
                ...
                switch (c) {
                    ...
                    default:
                        if (!Character.isLetterOrDigit(c)) {
                            ...
                            hashtagChangedListener.onChanged(PlexusSocialViewHelper.this, s.subSequence(
                                    indexOfPreviousNonLetterDigit(s, 0, start - 1) + 1, start
                            ));
                        } else if (mentionChangedListener != null && mentionEditing) {
                            mentionChangedListener.onChanged(PlexusSocialViewHelper.this, s.subSequence(
                                    indexOfPreviousNonLetterDigit(s, 0, start - 1) + 1, start
                            ));
                        }
                        break;
    ...
    /**
     * Configuring {@link PlexusSocialView} into given view.
     *
     * @param view TextView to install SocialView into.
     */
    public PlexusSocialViewHelper(@NonNull TextView view) {
        this(view, null);
    }
     ...
     */
    public PlexusSocialViewHelper(@NonNull TextView view, @Nullable AttributeSet attrs) {
        this.view = view;
        ...
        final TypedArray a = view.getContext().obtainStyledAttributes(
                attrs, R.styleable.PlexusSocialView, R.attr.socialViewStyle, R.style.Widget_PlexusSocialView
        );
        flags = a.getInteger(R.styleable.PlexusSocialView_PlexusSocialFlags, FLAG_HASHTAG | FLAG_MENTION | FLAG_HYPERLINK);
        hashtagColors = a.getColorStateList(R.styleable.PlexusSocialView_hashtagColor);
        mentionColors = a.getColorStateList(R.styleable.PlexusSocialView_mentionColor);
        hyperlinkColors = a.getColorStateList(R.styleable.PlexusSocialView_hyperlinkColor);
        a.recycle();
        recolorize();
    }
     ...
     * when {@link PlexusSocialView.OnClickListener} are activated.
     */
    private static class SocialClickableSpan extends ClickableSpan {
        ...
        public void onClick(@NonNull View widget) {
            if (!(widget instanceof PlexusSocialView)) {
                throw new IllegalStateException("Clicked widget is not an instance of SocialView.");
            }
            listener.onClick((PlexusSocialView) widget, !isHyperlink ? text.subSequence(1, text.length()) : text);
        }
