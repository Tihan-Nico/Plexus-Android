...
import com.plexus.slides.Slide;
import com.plexus.utils.DiffHelper;
import com.plexus.utils.MediaConstraints;
import com.plexus.utils.MediaUtil;
import com.plexus.utils.MessageUtil;
import com.plexus.utils.PlexusPreferences;
import com.plexus.utils.SingleLiveEvent;
...
class MediaSendViewModel extends ViewModel {
  ...
  private @Nullable User recipient;
...
/*
  ...
  void setRecipient(@Nullable User recipient) {
...
*/
  ...
  void onVideoBeginEdit(@NonNull Uri uri) {
    cancelUpload(new Media(uri, "", 0, 0, 0, 0, 0, false, false));
  }
  ...
  void onMeteredConnectivityStatusChanged(boolean metered) {
    ...
    preUploadEnabled  = shouldPreUpload(application, metered, recipient);
  }
  ...
  @NonNull LiveData<MediaSendActivityResult> onSendClicked(Map<Media, MediaTransform> modelsToTransform, @NonNull List<User> recipients, @NonNull List<Mention> mentions) {
    ...
    MediaRepository.transformMedia(application, initialMedia, modelsToTransform, (oldToNew) -> {
      ...
      if (isSms || MessageSender.isLocalSelfSend(application, recipient, isSms)) {
        ...
        result.postValue(MediaSendActivityResult.forTraditionalSend(recipient, updatedMedia, trimmedBody, isViewOnce(), trimmedMentions));
        return;
      }*/

      MessageUtil.SplitResult splitMessage = MessageUtil.getSplitMessage(application, trimmedBody, 0);
      ...
      /*uploadRepository.getPreUploadResults(uploadResults -> {
        *//*if (recipients.size() > 0) {
          ...
          result.postValue(MediaSendActivityResult.forPreUpload(recipient, uploadResults, splitBody, isViewOnce(), trimmedMentions));
        }*//*
      });*/
    });
  ...
  private void showViewOnceTooltipIfNecessary(@NonNull ViewOnceState viewOnceState) {
    /*if (viewOnceState == ViewOnceState.DISABLED && !PlexusPreferences.hasSeenViewOnceTooltip(application)) {
  ...
  @WorkerThread
  private void sendMessages(@NonNull List<User> recipients, @NonNull String body, @NonNull Collection<User> preUploadResults, @NonNull List<Mention> mentions) {
  ...
  private static boolean shouldPreUpload(@NonNull Context context, boolean metered, @Nullable User recipient) {
    /*return !metered && !isSms && !MessageSender.isLocalSelfSend(context, recipient);*/
    return false;
  }
