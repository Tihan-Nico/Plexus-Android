...
import com.plexus.jobmanagers.persistence.ConstraintSpec;
import com.plexus.jobmanagers.persistence.DependencySpec;
import com.plexus.jobmanagers.persistence.FullSpec;
...
class JobController {
    ...
    List<Job> onFailure(@NonNull Job job) {
    ...
    @WorkerThread
    synchronized @NonNull
    Job pullNextEligibleJobForExecution(@NonNull JobPredicate predicate) {
        try {
            Job job;
    ...
    @WorkerThread
    private @NonNull
    FullSpec buildFullSpec(@NonNull Job job, @NonNull Collection<String> dependsOn) {
        job.setRunAttempt(0);
    ...
    @WorkerThread
    private @Nullable
    Job getNextEligibleJobForExecution(@NonNull JobPredicate predicate) {
    ...
    private @NonNull
    Job createJob(@NonNull JobSpec jobSpec, @NonNull List<ConstraintSpec> constraintSpecs) {
    ...
    private @NonNull
    Job.Parameters buildJobParameters(@NonNull JobSpec jobSpec, @NonNull List<ConstraintSpec> constraintSpecs) {
    ...
    private @NonNull
    JobSpec mapToJobWithInputData(@NonNull JobSpec jobSpec, @NonNull Data inputData) {
