    ...
    int migrate(@NonNull JobStorage jobStorage, @NonNull Data.Serializer dataSerializer) {
        ...
        for (int i = lastSeenVersion; i < currentVersion; i++) {
            ...
            while (iter.hasNext()) {
                ...
                JobMigration.JobData originalJobData = new JobMigration.JobData(jobSpec.getFactoryKey(), jobSpec.getQueueKey(), data);
                JobMigration.JobData updatedJobData = migration.migrate(originalJobData);
