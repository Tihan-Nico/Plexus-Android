...
import com.plexus.stickers.StickerKeyboardRepository.PackListResult;
import com.plexus.utils.DeviceProperties;
...
import static com.plexus.stickers.StickerKeyboardPageFragment.*;
        ...
        EventListener
{
    ...
    private static class StickerPagerAdapter extends FragmentStatePagerAdapter {
        ...
        public Fragment getItem(int i) {
            ...
            if (i == 0) {
                fragment = newInstance(RECENT_PACK_ID);
            } else {
                StickerPackRecord pack = packs.get(i - 1);
                fragment = newInstance(pack.getPackId());
            }
        ...
        void setPacks(@NonNull List<StickerPackRecord> packs) {
            ...
            if (areListsEqual(this.packs, packs)) {
                itemPositions.put(RECENT_PACK_ID, 0);
    ...
    private static class IconProvider implements TabIconProvider {
        ...
        public void loadCategoryTabIcon(@NonNull GlideRequests glideRequests, @NonNull ImageView imageView, int index) {
            if (index == 0) {
                Drawable icon = ContextCompat.getDrawable(context, R.drawable.emoji_recent);
