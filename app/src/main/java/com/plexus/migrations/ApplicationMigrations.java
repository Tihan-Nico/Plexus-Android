...
public class ApplicationMigrations {
    ...
    private static final int LEGACY_CANONICAL_VERSION = 250;
    ...
    public static void onApplicationCreate(@NonNull Context context, @NonNull JobManager jobManager) {
        if (isLegacyUpdate(context)) {
            ...
            PlexusPreferences.setAppMigrationVersion(context, 0);
        }
        ...
        final int lastSeenVersion = PlexusPreferences.getAppMigrationVersion(context);
        ...
        if (migrationJobs.size() > 0) {
            ...
            EventBus.getDefault().register(new Object() {
                ...
                public void onMigrationComplete(MigrationCompleteEvent event) {
                    ...
                    PlexusPreferences.setAppMigrationVersion(context, event.getVersion());
            ...
            PlexusPreferences.setAppMigrationVersion(context, CURRENT_VERSION);
            VersionTracker.updateLastSeenVersion(context);
            UI_BLOCKING_MIGRATION_RUNNING.setValue(false);
        }
    }
    ...
    public static boolean isUpdate(@NonNull Context context) {
        return isLegacyUpdate(context) || PlexusPreferences.getAppMigrationVersion(context) < CURRENT_VERSION;
    }
