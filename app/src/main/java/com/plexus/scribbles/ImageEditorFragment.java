...
import com.plexus.imageeditor.ColorableRenderer;
...
import com.plexus.imageeditor.renderers.MultiLineTextRenderer;
import com.plexus.keyvalue.PlexusStore;
import com.plexus.media.PushMediaConstraints;
import com.plexus.mediaupload.MediaSendPageFragment;
...
import com.plexus.utils.SaveAttachmentTask;
...
public final class ImageEditorFragment extends Fragment implements ImageEditorHud.EventListener,
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    if (!PlexusStore.tooltipValues().hasSeenBlurHudIconTooltip()) {
      imageEditorHud.showBlurHudTooltip();
      PlexusStore.tooltipValues().markBlurHudIconTooltipSeen();
    }
  ...
  private void startTextEntityEditing(@NonNull EditorElement textElement, boolean selectAll) {
    imageEditorView.startTextEditing(textElement, PlexusPreferences.isIncognitoKeyboardEnabled(requireContext()), selectAll);
  }
    ...
    SimpleTask.run(getLifecycle(), () -> {
      if (mainImage.getRenderer() != null) {
        ...
        if (bitmap != null) {
          ...
          Bitmap render = model.render(PlexusDependencies.getApplication(), size);
          try {
  ...
  public void onSave() {
    SaveAttachmentTask.showWarningDialog(requireContext(), (dialogInterface, i) -> {
      ...
      Permissions.with(this)
                 ...
                 .ifNecessary()
                 .withPermanentDenialDialog(getString(R.string.MediaPreviewActivity_plexus_needs_the_storage_permission_in_order_to_write_to_external_storage_but_it_has_been_permanently_denied))
  ...
  private void performSaveToDisk() {
    SimpleTask.run(() -> {
      ...
      saveTask.executeOnExecutor(PlexusExecutors.BOUNDED, attachment);
    });
  }
