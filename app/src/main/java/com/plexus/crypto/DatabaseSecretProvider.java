...
public final class DatabaseSecretProvider {
    ...
    DatabaseSecret getOrCreate(@NonNull Context context) {
        String unencryptedSecret = PlexusPreferences.getDatabaseUnencryptedSecret(context);
        String encryptedSecret = PlexusPreferences.getDatabaseEncryptedSecret(context);
    ...
    private static @NonNull
    DatabaseSecret getUnencryptedDatabaseSecret(@NonNull Context context, @NonNull String unencryptedSecret) {
        try {
            ...
            if (Build.VERSION.SDK_INT < 23) {
                ...
                PlexusPreferences.setDatabaseEncryptedSecret(context, encryptedSecret.serialize());
                PlexusPreferences.setDatabaseUnencryptedSecret(context, null);
    ...
    private static @NonNull
    DatabaseSecret getEncryptedDatabaseSecret(@NonNull String serializedEncryptedSecret) {
        if (Build.VERSION.SDK_INT < 23) {
    ...
    private static @NonNull
    DatabaseSecret createAndStoreDatabaseSecret(@NonNull Context context) {
        ...
        if (Build.VERSION.SDK_INT >= 23) {
            ...
            PlexusPreferences.setDatabaseUnencryptedSecret(context, databaseSecret.asString());
        }
