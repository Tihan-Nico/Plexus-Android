...
final class PlexusCameraXModule {
    ...
    private final PlexusCameraView mCameraView;
    final AtomicBoolean mVideoIsRecording = new AtomicBoolean(false);
    private PlexusCameraView.CaptureMode mCaptureMode = PlexusCameraView.CaptureMode.IMAGE;
    private long mMaxVideoDuration = PlexusCameraView.INDEFINITE_VIDEO_DURATION;
    private long mMaxVideoSize = PlexusCameraView.INDEFINITE_VIDEO_SIZE;
    ...
    PlexusCameraXModule(PlexusCameraView view) {
        mCameraView = view;
    ...
    void bindToLifecycleAfterViewMeasured() {
        ...
        // Begin Plexus Custom Code Block
        int resolution = CameraXUtil.getIdealResolution(Resources.getSystem().getDisplayMetrics().widthPixels, Resources.getSystem().getDisplayMetrics().heightPixels);
        // End Plexus Custom Code Block

        Rational targetAspectRatio;
        if (getCaptureMode() == PlexusCameraView.CaptureMode.IMAGE) {
            // Begin Plexus Custom Code Block
        ...
        if (getCaptureMode() == PlexusCameraView.CaptureMode.IMAGE) {
            mCamera = mCameraProvider.bindToLifecycle(mCurrentLifecycle, cameraSelector,
                    mImageCapture,
                    mPreview);
        } else if (getCaptureMode() == PlexusCameraView.CaptureMode.VIDEO) {
            mCamera = mCameraProvider.bindToLifecycle(mCurrentLifecycle, cameraSelector,
                    mVideoCapture,
                    mPreview);
        } else {
    ...
    public void takePicture(Executor executor, OnImageCapturedCallback callback) {
        ...
        if (getCaptureMode() == PlexusCameraView.CaptureMode.VIDEO) {
    ...
    @NonNull
    public PlexusCameraView.CaptureMode getCaptureMode() {
        return mCaptureMode;
    }

    public void setCaptureMode(@NonNull PlexusCameraView.CaptureMode captureMode) {
