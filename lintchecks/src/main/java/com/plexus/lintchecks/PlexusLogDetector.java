...
public final class PlexusLogDetector extends Detector implements Detector.UastScanner {

  static final Issue LOG_NOT_PLEXUS = Issue.create("LogNotPlexus",
                                                   "Logging call to Android Log instead of Plexus's Logger",
                                                   "Plexus has its own logger which must be used.",
                                                   Category.MESSAGES,
                                                   5,
                                                   Severity.ERROR,
                                                   new Implementation(PlexusLogDetector.class, Scope.JAVA_FILE_SCOPE));

  static final Issue LOG_NOT_APP = Issue.create("LogNotAppPlexus",
                                                "Logging call to Plexus Service Log instead of App level Logger",
                                                "Plexus app layer has its own logger which must be used.",
                                                Category.MESSAGES,
                                                5,
                                                Severity.ERROR,
                                                new Implementation(PlexusLogDetector.class, Scope.JAVA_FILE_SCOPE));
  ...
  public void visitMethodCall(JavaContext context, @NotNull UCallExpression call, @NotNull PsiMethod method) {
    ...
    if (evaluator.isMemberInClass(method, "android.util.Log")) {
      ...
      context.report(LOG_NOT_PLEXUS, call, context.getLocation(call), "Using 'android.util.Log' instead of a Plexus Logger", fix);
    }

    if (evaluator.isMemberInClass(method, "com.plexus.core.utils.logging.Log")) {
      LintFix fix = quickFixIssueLog(call);
      context.report(LOG_NOT_APP, call, context.getLocation(call), "Using Plexus server logger instead of app level Logger", fix);
    }
  ...
  private LintFix quickFixIssueLog(@NotNull UCallExpression logCall) {
    ...
    String fixSource = "com.plexus.core.utils.logging.Log.";
